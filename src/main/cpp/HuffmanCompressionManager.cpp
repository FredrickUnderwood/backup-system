...
extern "C"
{
    JNIEXPORT void JNICALL Java_com_uestc_backupsystem_jni_HuffmanCompressionManager_compress(JNIEnv *env, jobject obj, jstring inputName, jstring outputName)
    {
        const char *inputNameS = env->GetStringUTFChars(inputName, nullptr);
        const char *outputNameS = env->GetStringUTFChars(outputName, nullptr);
        HuffmanCompressionManager::compress(inputNameS, outputNameS);
        env->ReleaseStringUTFChars(inputName, inputNameS);
        env->ReleaseStringUTFChars(outputName, outputNameS);
    }

    JNIEXPORT void JNICALL Java_com_uestc_backupsystem_jni_HuffmanCompressionManager_decompress(JNIEnv *env, jobject obj, jstring inputName, jstring outputName)
    {
        const char *inputNameS = env->GetStringUTFChars(inputName, nullptr);
        const char *outputNameS = env->GetStringUTFChars(outputName, nullptr);
        HuffmanCompressionManager::decompress(inputNameS, outputNameS);
        env->ReleaseStringUTFChars(inputName, inputNameS);
        env->ReleaseStringUTFChars(outputName, outputNameS);
    }
}